...
class Tos1Activity : BaseActivity<ActivityTos1Binding>(ActivityTos1Binding::inflate){
    ...
    private var agreeClickListener : AgreeClickListener? = null

    override fun initAfterBinding() {
        isSocial = intent.getBooleanExtra("isSocial", false)
        initClickListener()
        agreeClickListener = StepFiveFragment() as AgreeClickListener
    }

    interface AgreeClickListener{
        fun onAgreeClicked(agree:Boolean)
    }
    ...
    private fun initClickListener(){
        ...
        binding.tos1CheckBtn.setOnClickListener {
            if (isSocial){ // 소셜회원가입일때
                ...
                Log.d("cherry", "Y 설정해둠")
                agreeClickListener?.onAgreeClicked(true)
            }
            finish()
        }
    }
